...
bot_responses = {
    ...
    "как дела": "Чиназес",
    "пока": "бай бай",
    "пожалуйста": "потом проси",
    "как тебя зовут": "джокер"
}
...
def handle_client(conn, addr):
    ...
    while server_running:  # продолжаем, пока сервер работает
        try:
            ...
            print(f"Получено сообщение от {addr}: {decoded_message}")
            
            # Обработка сообщения чат-ботом
            for trigger, response in bot_responses.items():
                if trigger in decoded_message.lower():
                    conn.send(f"Бот: {response}".encode('utf-8'))
