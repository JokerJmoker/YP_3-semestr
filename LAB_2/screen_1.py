...
if __name__ == "__main__":
    ...
    while working:
        ...
        if show_help:
            poyline.draw_help(gameDisplay, len(points))
        ...
        if not pause:
            knot.set_points(points, speeds)  # Передаем аргументы в метод set_points
        if show_help:
            draw_help(gameDisplay, steps)  # Передаем аргумент steps
